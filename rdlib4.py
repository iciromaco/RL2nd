...
def getMaxAreaComponents(img, margincut=True):  
    ...
    contours, _hierarchy = cv2findContours34(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
...
def threeLinesSeq(src, showImage =  False, n_samples=16): 
    global tupple
    cnt,_hierarchy = cv2findContours34(src,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #  あらためて輪郭を抽出
